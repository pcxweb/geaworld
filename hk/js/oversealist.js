...
function getchooseInfo(num,countryID){
    $.ajax({
        ...
        success:function(data){
            ...
            var result = "";
            for (var i = 0; i < data.length; i++) {
                result += '<tr><td><a href="/hk/school-intro.html?id='+data[i].category_id+'&country='+countryID+'">'+data[i].title+'</a></td>'
                   + '<td><a href="/hk/school-intro.html?id='+data[i].category_id+'&country='+countryID+'">'+data[i].title_2+'</a></td></tr>'
            }
